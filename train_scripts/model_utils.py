...
def featurize(batch, device):
    ...
    mask_self = np.ones([B, L_max], dtype=np.int32)
    chain_encoding_all = np.zeros([B, L_max], dtype=np.int32)
    S = np.zeros([B, L_max], dtype=np.int32)
    L = np.zeros([B, L_max, 1], dtype=np.float32)
    ...
    for i,b in enumerate(batch):
        ...
        for step, letter in enumerate(all_chains):
            ...
            emb_file = b[f'seq_emb_file_chain_{letter}']
            emb_idx = b[f'seq_emb_idx_chain_{letter}']
            esm_emb = torch.load(emb_file)[name]

            esm_emb = esm_emb[emb_idx:] if emb_idx >= 0 else esm_emb[:emb_idx]
            mask_label = mask_label[emb_idx:] if emb_idx >= 0 else mask_label[:emb_idx]
            ...
            labels = b[f'probability_{letter}']
            sig_labels = np.argwhere(labels == 1).shape[0] + np.argwhere(labels == 0).shape[0]
            pos_labels = np.argwhere(labels == 1).shape[0]

            if pos_labels < sig_labels // 2:
                rands = np.argwhere(labels == 0).flatten().tolist()
                rands = random.sample(rands, sig_labels - pos_labels)
                mask_label[rands] = 0

            mask_self[i, l0:l1] = mask_label

            L[i, l0:l1, :] = np.expand_dims(labels, 0).transpose()
            ESM_S[i, l0:l1, :] = esm_emb[l0:l1,:]
            l0 += chain_length
            c += 1
    ...
    return X, S, ESM_S, L, mask, lengths, residue_idx, mask_self, chain_encoding_all, batch_name

def loss_mse(label, predictions, mask):
    criterion = torch.nn.MSELoss(reduction='none')

    loss = criterion(
        predictions.contiguous().view(-1, predictions.size(-1)), label.contiguous().view(-1, predictions.size(-1))
    ).view(label.shape[0], label.shape[1])
    ...
    def _aapro(self, S:torch.Tensor):
        ...
        for i,seq in enumerate(S):
            for j,aa in enumerate(seq):
                if aa != 0:
                    aapro = self.aa_dict[alphabet[aa]]
...
class SplitSeek(nn.Module):
    def __init__(self, aaindex1, num_letters=21, node_features=128, edge_features=128, 
        ...
        self.W_out1 = nn.Linear(hidden_dim, hidden_dim // 2, bias=True)
        self.W_out2 = nn.Linear(hidden_dim // 2, 1, bias=True)
